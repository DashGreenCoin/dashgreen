...
#include "ui_zdashgreencontroldialog.h"
...
std::list<std::string> ZDashGreenControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDashGreenControlDialog::listMints;

ZDashGreenControlDialog::ZDashGreenControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDashGreenControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDashGreenControlDialog::~ZDashGreenControlDialog()
{
    delete ui;
}

void ZDashGreenControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDashGreenControlDialog::updateList()
{
...
void ZDashGreenControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDashGreenControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDashGreen_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDashGreenControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDashGreenControlDialog::GetSelectedMints()
{
...
void ZDashGreenControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
