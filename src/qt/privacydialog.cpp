...
#include "zdashgreencontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDASHG ought to be enough for anybody." - Bill Gates, 2017
    ui->zDASHGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDASHGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDASHG->setEnabled(false);
        ui->pushButtonMintzDASHG->setToolTip(tr("zDASHG is currently disabled due to maintenance."));

        ui->pushButtonSpendzDASHG->setEnabled(false);
        ui->pushButtonSpendzDASHG->setToolTip(tr("zDASHG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDASHGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDASHG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDASHG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDASHG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDASHG in ") +
...
void PrivacyDialog::on_pushButtonSpendzDASHG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDASHG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDASHG
        sendzDASHG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDASHG
    sendzDASHG();
}

void PrivacyDialog::on_pushButtonZDashGreenControl_clicked()
{
    ZDashGreenControlDialog* zDashGreenControl = new ZDashGreenControlDialog(this);
    zDashGreenControl->setModel(walletModel);
    zDashGreenControl->exec();
}

void PrivacyDialog::setZDashGreenControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDashGreenSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDASHG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DashGreen Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDASHGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDASHG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DASHG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDASHG</b>";
    ...
    // use mints from zDashGreen selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDashGreenControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDashGreenControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdashgreen selector in case it was used
    ZDashGreenControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDashGreen Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DashGreen, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDashGreen Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDASHG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDASHG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDASHG "));
    ui->labelzDASHGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDASHGSyncStatus->setVisible(fShow);
}
