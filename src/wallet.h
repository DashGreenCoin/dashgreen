...
// 33223 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_6666 = 33223;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DASHG at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDASHG send
enum ZerocoinSpendStatus {
    ZDASHG_SPEND_OKAY = 0,                            // No error
    ZDASHG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDASHG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDASHG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDASHG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDASHG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDASHG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDASHG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDASHG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDASHG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDASHG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDASHG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDASHG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDASHG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDASHG_SPENT_USED_ZDASHG = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZDashGreenAutoBackups(bool fEnabled)
    {
